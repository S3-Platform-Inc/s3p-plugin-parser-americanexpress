...
config = PluginConfig(
    plugin=CoreConfig(
        reference='americanexpress',         # уникальное имя источника
        type=SOURCE,                            # Тип источника (SOURCE, ML, PIPELINE)
        files=['americanexpress.py', ],        # Список файлов, которые будут использоваться в плагине (эти файлы будут сохраняться в платформе)
    ...
    payload=payload.PayloadConfig(
        file='americanexpress.py',                 # python файл плагина (точка входа). Этот файл должен быть указан в `plugin.files[*]`
        classname='AMERICANEXPRESS',               # имя python класса в указанном файле
